...
class TestDockerSecuritySimple:
    ...
    def test_command_validation_comprehensive(self, docker_judge):
        ...
        for cmd in edge_cases:
            # 空命令和只有空格的命令应该被拒绝
            if cmd.strip() == "":
                assert not docker_judge.validate_command(
                    cmd
                ), f"应该拒绝空命令: '{cmd}'"
            # 包含危险操作的命令应该被拒绝
...
int main() {
    ...
    def test_output_normalization(self, docker_judge):
        ...
        # 测试混合情况
        assert (
            docker_judge._normalize_output("  Hello, World!  \n\n  ") == "Hello, World!"
        )
        ...
        assert (
            docker_judge._normalize_output(multi_line_spaces)
            == "Line 1\nLine 2\nLine 3"
        )
